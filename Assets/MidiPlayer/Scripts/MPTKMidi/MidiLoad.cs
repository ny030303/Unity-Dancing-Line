    ...
    public partial class MidiLoad
    {
        ...
        private void fluid_player_set_midi_tempo(int MPQN)
        {
            ...
            else
            {
                ...
                //Debug.Log($"Apres tempo change tempo:{MPQN} BPM:{MPTK_MPQN2BPM(MPQN)} pulse:{MPTK_PulseLenght} start:{start_msec} ms start:{start_ticks} ticks");
                start_msec = cur_msec;
            ...
            try
            {
                if (next_event >= 0 && MPTK_MidiEvents != null /*v2.9.0*/)
                {
                    ...
                    while (true)
                    {
                        ...
                        MPTKEvent trackEvent = MPTK_MidiEvents[next_event];
                        // 2.9.0 remove stupid shift half quanta
                        long quantizedTick = Quantization != 0 ? ((trackEvent.Tick /*+ Quantization / 2*/) / Quantization) * Quantization : trackEvent.Tick;

                        if (quantizedTick >= ticks) // V2.872 replace > by >=
                        {
                            break;
                        }
                        //if (seek_ticks >= 0 &&    v2.9.0 gentle optimization ;-)
                        //   (trackEvent.Command == MPTKCommand.NoteOn ||
                        //    trackEvent.Command == MPTKCommand.NoteOff ||
                        //    (trackEvent.Command == MPTKCommand.MetaEvent && trackEvent.Meta != MPTKMeta.SetTempo)))
                        //{
                        //}
                        //else
                        if (seek_ticks < 0 ||
                                ...
                                !(trackEvent.Command == MPTKCommand.MetaEvent && trackEvent.Meta != MPTKMeta.SetTempo)))
                        {
                            ...
                            // Removed from v2.9.0 MPTKEvent mptkEvent = ConvertTrackEventToMPTKEvent(trackEvent);

                            if (trackEvent.Meta == MPTKMeta.SetTempo && MPTK_EnableChangeTempo)
                            {
                                MPTK_MicrosecondsPerQuarterNote = trackEvent.Value;
                                fluid_player_set_midi_tempo(MPTK_MicrosecondsPerQuarterNote);
                            }
                            trackEvent.IdSession = idSession;
                            midiEvents.Add(trackEvent);
                        }
                        next_event++;
                    }
