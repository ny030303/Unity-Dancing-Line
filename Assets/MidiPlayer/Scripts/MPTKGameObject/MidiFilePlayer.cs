...
namespace MidiPlayerTK
{
    ...
    public partial class MidiFilePlayer : MidiSynth
    {
        ...
        /// <summary>@brief 
        /// When the value is true, playing stop at the last note found, the note is played but the MIDI sequencer don't wait the end of the note to stop.\n
        /// With false, the MIDI sequencer stops only when any note are playing even in release phase.\n
        /// @version 2.9.0
        /// @note
        /// @li if #MPTK_KeepEndTrack if set and a End Track (or any events except note-on) exists in the MIDI, playing will stop at this last event (if all notes are released).
        /// @li No impact on #MPTK_Duration that remains the same (including the duration of the last note).
        /// </summary>
        [HideInInspector]
        public bool MPTK_StopPlayAtLastNote;
        ...
        /// <summary>@brief 
        /// Set or get the current position in milliseconds.
        /// @note
        /// @li Works only when the MIDI is playing.
        /// @li You can't set the position before or just after MPTK_Play() because MPTK_Play() reset the position at the start.
        /// @li Rather, set the position when the event OnEventStartPlayMidi() is triggered. See example bellow.
        /// @li Look also the properties #MPTK_TickCurrent to change the position by MIDI ticks.
        /// @li When the MIDI is playing look at the inspector of the MidiFilePlayer prefab to read (or change) the current position in seconds and find the position you want.
        /// @par
        /// @details
        ...
        /// @li works only when the MIDI is playing.\n
        /// @li you can't set the position before or just after MPTK_Play() because MPTK_Play() reset the position at the start.\n
        /// @li rather, set  the position when the event OnEventStartPlayMidi() is triggereed. See example below.\n
        /// @li look also the properties #MPTK_Position to change the position by milliseconds.\n
        /// @li when the MIDI is playing look at the inspector of the MidiFilePlayer prefab to read (or change) the current tick and find the position you want.\n
        /// @par
        /// See example:\n
        /// @code
        /// public MidiFilePlayer midiFilePlayer;
            ...
            if (midiLoaded != null)
            {
                ...
                do
                {
                    ...
                    do
                    {
                        ...
                        {
                            if (MPTK_StopPlayAtLastNote || MPTK_StatVoiceCountActive <= 0)
                            {
                                //Debug.Log("EndMidiEvent");
                                midiLoaded.ReadyToPlay = false;
                                break;
                            }
                        }

                        if (replayMidi || stopMidi || (toPosition > 0 && toPosition > fromPosition && MPTK_Position > toPosition))
                        {
                            midiLoaded.ReadyToPlay = false;
                            break;
                        }
