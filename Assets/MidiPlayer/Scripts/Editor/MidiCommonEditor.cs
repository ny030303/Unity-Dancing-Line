...
namespace MidiPlayerTK
{
    ...
    public class MidiCommonEditor: ScriptableObject
    {
        ...
        public void MidiFileParameters(MidiFilePlayer instance)
        {
            ...
            tooltip = "If set, playing stop at the last note found, the note is played but the MIDI sequencer don't wait the end of the note to stop.\n";
            tooltip += "If not set, playing stop at the last note found but when all playing notes are released.";
            instance.MPTK_StopPlayAtLastNote= EditorGUILayout.Toggle(new GUIContent("Stop Play At Last Note", tooltip), instance.MPTK_StopPlayAtLastNote);
