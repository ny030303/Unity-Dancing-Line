...
public class MidiLoop : MonoBehaviour
{
    ...
    public void MidiReadEvents(List<MPTKEvent> midiEvents)
    {
        ...
        // they are already in the pipeline of the MIDI synth or even already playing!
        ...
        midiEvents.ForEach(midiEvent => { if (midiEvent.Command == MPTKCommand.NoteOn) Debug.Log($"     Tick:{midiEvent.Tick} Command:{midiEvent.Command} Value:{midiEvent.Value}"); });

        if (midiFilePlayer.MPTK_TickCurrent >= ConvertBarToTick(LoopBar) - 1)
        {
            Debug.Log($"     <color=red>Replay at tick {ConvertBarToTick(LoopBar)}</color>");
            ...
            // Delayed start for 2 seconds with a volume rampup of 0.1 second (just for fun).
            // StartPlay method will be re-triggered with a move to the start bar when setting MPTK_TickCurrent
